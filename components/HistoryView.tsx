...
const HistoryView: React.FC<Props> = ({
  ...
  const visibleRecordCount = filteredRecords.filter(r => !(r.type === 'CASHFLOW' && r.isTargetRecord)).length;
  const hasActiveFilters = Boolean(
    filterAccount || filterTicker || filterDateFrom || filterDateTo || !includeCashFlow
  );
    ...
    <>
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              ...
              className="bg-purple-600 text-white px-3 py-1.5 rounded text-sm hover:bg-purple-700 shadow-lg shadow-purple-600/20 whitespace-nowrap"
            >
              {tr.history.batchUpdateMarket}
            </button>
            <button
              onClick={onOpenImport}
              className="bg-indigo-50 text-indigo-600 px-3 py-1.5 rounded text-sm hover:bg-indigo-100 border border-indigo-200 whitespace-nowrap"
            >
      ...
      {!showFilterPanel && (
        <div className="text-sm text-slate-600 px-1">
          {translate('history.showingRecords', language, { count: visibleRecordCount })}
          {hasActiveFilters && (
            <button
              type="button"
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
            {filteredRecords.map(record => {
              ...
              return (
                ...
                >
                  <td className="px-2 sm:px-3 py-2 whitespace-nowrap text-slate-600 dark:text-slate-300">
                    {record.date}
                  </td>
                  <td className="px-2 sm:px-3 py-2 text-slate-500 dark:text-slate-400 hidden sm:table-cell">
                    {accName}
                  </td>
                  <td className="px-2 sm:px-3 py-2 font-semibold text-slate-700 dark:text-slate-100">
                    {record.type === 'TRANSACTION' ? (
                      <div className="flex flex-col">
                        <span>
                          <span className="text-xs text-slate-400 dark:text-slate-500 mr-1">{record.market}</span>
                          {record.ticker}
                        </span>
                        <span className="text-sm text-slate-400 dark:text-slate-500 sm:hidden">{accName}</span>
                      </div>
                    ) : (
                      <div className="flex flex-col">
                        {(() => {
                        ...
                        {tAN && record.subType === 'TRANSFER' && (
                          <span className="text-xs text-slate-400">→ {tAN}</span>
                        )}
                  ...
                  <td className="px-2 sm:px-3 py-2 hidden md:table-cell">
                    <span className={`px-2 py-0.5 rounded text-xs font-bold ${bc}`}>{dt}</span>
                  </td>
                  <td className="px-2 sm:px-3 py-2 text-right font-mono text-slate-600 dark:text-slate-300">
                    {record.type === 'TRANSACTION'
                  ...
                  <td className="px-2 sm:px-3 py-2 text-right font-bold font-mono text-slate-700 dark:text-slate-100">
                    {formatAmount(record.amount)}
                    <div className="md:hidden mt-0.5">
                      <span className={`text-sm font-normal ${nBal >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {formatAmount(nBal)}
                      </span>
                    </div>
                  </td>
                  <td className="px-2 sm:px-3 py-2 text-right hidden md:table-cell">
                    <div className="flex flex-col items-end">
                      <span className={`font-medium ${nBal >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {formatAmount(nBal)}
                      </span>
                      ...
                      <div className="flex flex-col sm:flex-row gap-1 sm:gap-2 justify-end items-end sm:items-center">
                        {record.type === 'TRANSACTION' && (
                          <button
                            ...
                            className="text-blue-400 hover:text-blue-600 text-sm px-1.5 sm:px-2 py-0.5 sm:py-1 border border-blue-100 rounded hover:bg-blue-50 whitespace-nowrap"
                          >
                            {tr.history.edit}
                          </button>
                        )}
                        <button
                          ...
                          className="text-red-400 hover:text-red-600 text-sm px-1.5 sm:px-2 py-0.5 sm:py-1 border border-red-100 rounded hover:bg-red-50 whitespace-nowrap"
                        >
                          {tr.history.delete}
                        </button>
                      </div>
